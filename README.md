...
- **認證**：JWT Bearer + Google OAuth
- **安全**：Token 黑名單（ConcurrentDictionary）
...
│   ├── admin/          後台管理元件（待完成）
│   └── pages/          HomePage, StorePage, DetailPage, CartPage, OrdersPage, ProfilePage, AuthCallback
│       └── admin/      AdminPage, AdminProductPage, AdminOrderPage, AdminUserPage, AdminReviewPage
├── context/
...
│   ├── useProducts.js  useProducts, useCategories, useProductDetail
...
- Google OAuth Callback URL 需在 Google Cloud Console 設定
